Our method of solution transforms the
\hyperref[eq:helmholtz_equation]{Helmholtz equation} from a differential
equation into an \textit{integral equation}. If we know
$U^{\prime}(x^{\prime},\,y^{\prime},\,0)$ for each
$(x^{\prime},\,y^{\prime})$ in the $z=0$ plane, then the
solution for $U(x,\,y,\,z)$ is:

\begin{equation}
    \def\arraystretch{2.5em}
    \begin{array}{rcl}
        \displaystyle
        U(\mathbf{r})
        \!\!
        &=&
        \!\!
        \displaystyle
        \frac{1}{i\lambda}
        \iint_{\mathbb{R}^{2}}
            U^{\prime}(x^{\prime},\,y^{\prime},\,0)
            \frac{z\exp\left(ikr\right)}{r^{2}}
            \left(
                1-\frac{1}{ikr}
            \right)\,\textrm{d}x^{\prime}\,\textrm{d}y^{\prime}\\
        \!\!
        &=&
        \!\!
        \displaystyle
        \frac{1}{i\lambda}
        \int_{0}^{\infty}\int_{0}^{2\pi}
            U^{\prime}(\rho^{\prime},\,\phi^{\prime})
            \frac{z\rho^{\prime}\exp\left(ikr\right)}{r^{2}}
            \left(
                1-\frac{1}{ikr}
            \right)\,\textrm{d}\phi^{\prime}\textrm{d}\rho^{\prime}\\
    \end{array}
\end{equation}

where this second integral simply swaps from Cartesian
coordinates to cylindrical, and where $r$ is given by:

\begin{equation}
    \def\arraystretch{2em}
    \begin{array}{rcl}
        \displaystyle
        r
        \!\!
        &=&
        \!\!
        \displaystyle
        \|\mathbf{r}-\mathbf{r}^{\prime}\|\\
        \!\!
        &=&
        \!\!
        \displaystyle
        \|(x-x^{\prime},\,y-y^{\prime},\,z)\|\\
        \!\!
        &=&
        \!\!
        \displaystyle
        \sqrt{(x-x^{\prime})^{2}+(y-y^{\prime})^{2}+z^{2}}
    \end{array}
\end{equation}

The parameter $\lambda$ is the \textit{wavelength} of the wave, and
$k$ is the \textit{wavenumber}:

\begin{equation}
    k
    =
    \frac{2\pi}{\lambda}
\end{equation}

In practice, $\lambda$ is very small, meaning $k$ is quite large.
Since $r$ is also big in most circumstances, the contribution
to the integral from the $1/kr$ term is negligible and may be
omitted for numerical purposes. Our approximate solution is then:

\begin{equation}
    \def\arraystretch{2.5em}
    \begin{array}{rcl}
        \displaystyle
        U(\mathbf{r})
        \!\!
        &\approx&
        \!\!
        \displaystyle
        \frac{1}{i\lambda}
        \iint_{\mathbb{R}^{2}}
            U^{\prime}(x^{\prime},\,y^{\prime},\,0)
            \frac{z\exp\left(ikr\right)}{r^{2}}
                \,\textrm{d}x^{\prime}\,\textrm{d}y^{\prime}\\
        \!\!
        &=&
        \!\!
        \displaystyle
        \frac{1}{i\lambda}
        \int_{0}^{\infty}\int_{0}^{2\pi}
            U^{\prime}(\rho^{\prime},\,\phi^{\prime})
            \frac{z\rho^{\prime}\exp\left(ikr\right)}{r^{2}}
                \,\textrm{d}\phi^{\prime}\textrm{d}\rho^{\prime}\\
    \end{array}
\end{equation}

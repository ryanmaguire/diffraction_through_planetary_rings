It would be great if we could simply \textit{solve} the three dimensional
wave equation and obtain a nice formula for $\mathbf{E}$ and $\mathbf{B}$,
but such a problem isn't even \textit{well-posed}.
\footnote{
    Just like asking which function $y$ satisfies
    $y^{\prime\prime}(x)=y(x)$. There are infinitely many such functions,
    but if we are given the values of $y(x_{0})$ and $y^{\prime}(x_{0})$
    for some fixed real number $x_{0}$, then we may solve this equation
    uniquely. The wave equation is much harder since it involves partial
    derivatives in four variables.
}
The solutions to these equations are highly sensitive to the
\textit{initial conditions}, which are
...
to model heat transfer. This was accomplished by Joseph Fourier
(1768 -- 1830 C.E.) in 1822, and his methods led to the discovery of
\textit{Fourier analysis}, \textit{Fourier series}, and
\textit{Fourier transforms} [\cite{FourierAnalyticalTheoryOfHeat}].
Astronomers and physicists make frequent use of these ideas,
and the equations we will describe later
concern the field of \textit{Fourier optics}
[\cite{GoodmanIntroductionToFourierOptics}].

Fourier considers a bar of finite length $L$
[\cite[p.~1-22]{GonzalezVelascoFourierAnalysisAndBoundaryValueProblems}].
The left and right endpoints are held to fixed temperatures,
$T_{L}$ and $T_{R}$, respectively, and we do not require that $T_{L}=T_{R}$.
...
The \hyperref[eq:heat_equation]{heat equation} then becomes
(the $\Longrightarrow$ symbols reads as \textit{implies}):
...
\textit{system of ordinary differential equations}.

\begin{equation}
    \def\arraystretch{2.5em}
    \label{eq:heat_equation_separation_of_variables}
    \begin{array}{rcl}
        \displaystyle
        \frac{\textrm{d}T}{\textrm{d}t}(t)
        +
...
\begin{equation}
    \label{eq:heat_equation_simple_solution}
    u(x,\,t)
    =
...
probably \textit{not} satisfy $u(x,\,t_{0})=f(x)$, our smooth function
...
\textit{linear partial differential equation}. He then proposed summing
together \textit{infinitely many} of these partial solutions to obtain the
solution $u$ that satisfies $u(x,\,t_{0})=f(x)$. That is:

\begin{equation}
    \label{eq:heat_equation_fourier_series}
    u(x,\,t)
    =
    \sum_{n=1}^{\infty}a_{n}
...
\textit{any} smooth function defined on the closed interval $[0,\,L]$
could be represented by a sum of sines. This is where
\textit{Fourier analysis} begins.

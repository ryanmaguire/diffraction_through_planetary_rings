...
There are many ways to do this \textit{directly}, but
first let's do it \textit{indirectly} by approximating bits of this equation.
Suppose the observer at the point $\mathbf{r}=(x,\,y,\,z)$
is far away from the plane, so that
$z\gg\|(x^{\prime},\,y^{\prime},\,0)\|$ for all $(x^{\prime},\,y^{\prime})$
where $U^{\prime}$ is non-zero.
...
If $z$ is much larger than the diameter of $R$ (where \textit{diameter}
...
$y^{\prime}$. We may treat the ratios $x^{\prime\,2}/z$ and
...
requirement is that $z$ is much greater than the \textit{characteristic scale}
of the region $R$ where $U^{\prime}$ is non-zero (we referred to this as the
\textit{diameter} in a previous paragraph). The physical interpretation is that
$U^{\prime}$ describes an \textit{aperture}, and where it is non-zero is
where light is allowed to pass through.
Points $(x^{\prime},\,y^{\prime})$ satisfying
$U^{\prime}(x^{\prime},\,y^{\prime},\,0)=0$ are those where light is
completely blocked.

The \textbf{Fraunhofer Diffraction Formula} is then obtained from the
...
The \textit{intensity} of the solution is the square of the magnitude of
$U$ (possibly with some \textit{normalizing factor}, depending on author).
We have:
...
\textit{intensity formula in the Fraunhofer limit}.
